...
//  BuildMall
//
//  Created by 51wood on 2019/1/17.
...
@interface DynamicVC () <SDTimeLineCellDelegate, UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource,DynamicChooseViewDelegate>
...
-(void)photoView:(DynamicChooseView *)photoView selectBtn:(UIButton *)btn
{
    ...
    if ([btn.titleLabel.text isEqualToString:@"图文选择"]) {
        SendDynamicVC* VC = [SendDynamicVC new];
        VC.isPhoto = YES;
...
-(void)onDone
{
    DynamicChooseView *photoView = [DynamicChooseView dynamicChooseView];
